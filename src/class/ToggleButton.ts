export class ToggleButton extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    this.text = new Phaser.GameObjects.Text(this.scene, 0, 0, text, textStyle).setScale(0.5).setOrigin(0.5)
    this.add(this.text)
    ...
    this.leftZone = scene.add.zone(0, 0, 100, 60).setOrigin(1, 0.5).setInteractive({
      useHandCursor: true,
    })
    this.add(this.leftZone)
    this.rightZone = scene.add.zone(0, 0, 100, 60).setOrigin(0, 0.5).setInteractive({
      useHandCursor: true,
    })
